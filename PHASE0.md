...
- `tick()` and `reset_animation_clock_for_testing()` are intentionally exposed in the public interface even though only Z05 and the integration test should call them. The alternative — splitting them into a "private" header — is more ceremony than it's worth for two functions. The comments document the intent.
